...
def th_labels(th) -> list[str]:
    """Подписи колонки <th>. В отрендеренном DOM там сидит SVG-иконка сортировки
    с <title>Created with Sketch.</title>, поэтому берём только текстовые узлы
    и первую строку, а не get_text целиком (иначе сравнение заголовков ломается)."""
    out = []
...
def parse_tx_table(html: str) -> list[Transaction]:
    ...
    for table in soup.find_all("table"):
        heads = [" ".join(th_labels(th)) for th in table.find_all("th")]
        if len(heads) < len(TX_HEADERS):
            continue
        # сделки, а не начисления: сверяем префикс подписи каждой колонки
        if any(not h.startswith(want) for h, want in zip(heads, TX_HEADERS)):
            continue
...
def describe_html(html: str) -> str:
    """Одна строка с тем, ЧТО реально пришло. Нужна, чтобы не гадать по логу."""
    if not html:
    ...
    parts = [f"длина={len(html)}", f"title={title!r}",
             f"таблиц={len(tables)}", f"строк в них={rows}",
             "__NEXT_DATA__=" + ("есть" if "__NEXT_DATA__" in html else "НЕТ"),
             "заглушка='Здесь пока что пусто'=" + ("есть" if "пока что пусто" in html else "нет")]
    for marker, label in (("captcha", "похоже на капчу"), ("Доступ ограничен", "доступ ограничен"),
                          ("Attention Required", "часовая проверка (Cloudflare)"),
                          ("403 Forbidden", "403")):
        if marker.lower() in html.lower()[:6000]:
            parts.append(label)
    return "; ".join(parts)
...
class Site:
    ...
    def fetch_html_browser(self, save_debug_to: str | None = None) -> str:
        """Рендерит страницу в headless-Chromium и дожидается РЕАЛЬНЫХ строк.

        Обязательно: переход на вкладку #transactions и wait по содержимому.
        `networkidle` тут не годится — таблица сделок догружается только после
        клика по табу, поэтому без явного ожидания получаем пустую выдачу.
        """
        ...
        with sync_playwright() as pw:
            ...
            ctx = b.new_context(user_agent=UA, locale="ru-RU",
                                timezone_id="Europe/Moscow",
                                viewport={"width": 1600, "height": 1200})
            pg = ctx.new_page()
            try:
                pg.goto(self.page_url + "#transactions",
                        wait_until="domcontentloaded", timeout=90_000)
                try:
                    pg.wait_for_load_state("networkidle", timeout=20_000)
                except Exception:  # noqa: BLE001
                    pass            # networkidle может не наступить из- счётчиков/вебсокетов
                # Явно кликаем таб «Сделки» (link с href="#transactions"), если он есть
                ...
                try:
                    pg.wait_for_function(
                        """() => {
                            ...
                            for (const r of rows) {
                                const t = r.innerText || '';
                                if (/Покупка|Продажа/.test(t) && r.querySelectorAll('td').length >= 8)
                                    return true;
                            }
                            return false;
                        }""", timeout=25_000)
                except Exception as e:  # noqa: BLE001
                    log.debug("ожидание строк сделок: %s", e)
                html = pg.content()
                if save_debug_to:
                    try:
                        pg.screenshot(path=str(Path(save_debug_to).with_suffix(".png")),
                                     full_page=False)
                    except Exception:  # noqa: BLE001
                        pass
            finally:
    ...
    # ---- HTML ------------------------------------------------------------- #
    def fetch_via_html(self, attempts: int = 2) -> tuple[list[Transaction], dict]:
        """Браузер → обычный HTTP, с повтором. Возвращает ещё и диагностику."""
        last_err = ""
        for try_no in range(1, max(1, attempts) + 1):
            html, how = self._try_get_html()
            if not html:
                continue
            meta = parse_next_data(html)
            txs = parse_tx_table(html)
            for t in txs:
                t.portfolio = meta.get("name") or ""
            if txs:
                log.info("Получено %d сделок (%s, попытка %d)", len(txs), how, try_no)
                meta["_source_kind"] = how
                meta["_html_len"] = len(html)
                return txs, meta
            last_err = describe_html(html)
            log.warning("Попытка %d (%s): строк нет. Что в ответе: %s",
                        try_no, how, last_err)
            self._save_debug(html, how)
            if how == "browser":
                # в браузерном рендере пусто — пробуем ещё раз, возможно гонка загрузок
                time.sleep(2)
                continue
            break
        return [], {"_diagnostics": last_err or "ни браузер, ни HTTP не вернули HTML",
                    "_last_error": last_err}

    def _save_debug(self, html: str, how: str) -> None:
        """Сбрасываем полученный HTML рядом с state-файлом: по нему видно,
        что именно вернул сайт (капча? редирект? пустая таблица?)."""
        try:
            d = Path(self.debug_dir) if getattr(self, "debug_dir", None) else Path(".")
            d.mkdir(parents=True, exist_ok=True)
            (d / "debug_last.html").write_text(html, encoding="utf-8")
            (d / "debug_last.txt").write_text(f"how={how}\n{describe_html(html)}\n",
                                               encoding="utf-8")
        except Exception as e:  # noqa: BLE001
            log.debug("не смог сохранить диагностику: %s", e)

    def _try_get_html(self) -> tuple[str, str]:
        if self.use_browser:
            try:
                return self.fetch_html_browser(), "browser"
            except Exception as e:  # noqa: BLE001
                log.warning("Playwright не поднялся (%s), пробую обычный HTTP", e)
        try:
            return self.fetch_html_http(), "http"
        except Exception as e:  # noqa: BLE001
            log.warning("И обычный HTTP не удался: %s", e)
            return "", "none"
...
class Bot:
    def __init__(self, cfg: dict):
        ...
        self.site = Site(
            ...
            debug_dir=str(Path(cfg["state_file"]).parent),
        )
    ...
    def check_once(self) -> int:
        try:
            ...
            self.state.save()
            return -1
        # ВАЖНО: снимок «до» нужен до update_meta — иначе сравнение счётчиков
        # всегда давало бы «не выросло», и алерт о новых сделках не сработал бы.
        prev_cnt = self.state.data["meta"].get("transactions_count")
        ...
        if not txs:
            ...
            # Но часть данных всё равно доходит: SSR-блок __NEXT_DATA__ содержит
            # transactionsCount. Поэтому: (1) предупреждаем с диагностикой,
            # (2) если счётчик подрос — шлём сигнал «сделки появились, деталей нет».
            diag = meta.get("_diagnostics") or meta.get("_last_error") or "нет данных"
            msg = ("Не удалось разобрать таблицу сделок.\n"
                   f"source={self.cfg['source']}, browser={self.cfg['use_browser']}\n"
                   f"Что пришло в ответе: {diag}\n\n"
                   "Частые причины: сайт закрыл доступ для дата-центров (капча), "
                   "сменилась вёрстка, или Chromium не стартовал в среде запуска.")
            self.state.data["meta"]["last_error"] = "empty result"

            cur_cnt = meta.get("transactions_count")
            # счётчик до начала мониторинга может быть неизвестен — тогда молчим
            alert_count = (isinstance(cur_cnt, int) and isinstance(prev_cnt, int)
                           and cur_cnt > prev_cnt)
            if alert_count:
                self.broadcast(
                    "🔔 <b>В портфеле появились сделки</b>\n\n"
                    f"Сделок всего: {prev_cnt} → <b>{cur_cnt}</b> "
                    f"(+{cur_cnt - prev_cnt})\n"
                    f"Последняя дата сделки: {esc(str(meta.get('last_transaction_date')))}\n\n"
                    "⚠️ Детали вытащить не удалось, см. лог запуска и \n"
                    f'<a href="{self.site.page_url}#transactions">сделки на сайте</a>')
                log.warning("счётчик вырос (%s → %s), но детали недоступны", prev_cnt, cur_cnt)
                self.state.update_meta(meta)
                self.state.save()
                return 0
...
def main() -> int:
    ...
    ap.add_argument("--diagnose", action="store_true",
                    help="показать, что реально возвращает сайт (для отладки в Actions)")
    ...
    if a.diagnose:
        site = bot.site
        html, how = site._try_get_html()
        log.info("получено способом: %s", how)
        print("Диагностика ответа:", describe_html(html))
        meta = parse_next_data(html)
        print("SSR-метаданные:", meta or "не найдены (плохо)")
        txs = parse_tx_table(html)
        print(f"Строк сделок распарсено: {len(txs)}")
        if site.use_browser:
            print("\nПробую браузерный рендер…")
            try:
                h2 = site.fetch_html_browser()
                print("browser:", describe_html(h2))
                print("Строк через браузер:", len(parse_tx_table(h2)))
            except Exception as e:  # noqa: BLE001
                print(f"браузер не поднялся: {type(e).__name__}: {e}")
        return 0
